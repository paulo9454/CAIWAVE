...
def build_bundle(nat_enabled=True):
    router = {
        "id": "router-1",
        "name": "GOODlife",
        "owner_id": "owner-1",
...
def test_validates_good_bundle():
    result = validate_provisioning_bundle(build_bundle())

    assert result.valid is True
    assert result.errors == []


def test_detects_firewall_wan_mismatch():
    bundle = build_bundle()
    bad_firewall = bundle.firewall.model_copy(update={"wan_interface": "wrong"})
    bad_bundle = bundle.model_copy(update={"firewall": bad_firewall})

    result = validate_provisioning_bundle(bad_bundle)

    assert result.valid is False
    assert "Firewall WAN interface does not match topology upstream" in result.errors


def test_detects_missing_radius_firewall_host():
    bundle = build_bundle()
    bad_firewall = bundle.firewall.model_copy(update={"radius_hosts": []})
    bad_bundle = bundle.model_copy(update={"firewall": bad_firewall})

    result = validate_provisioning_bundle(bad_bundle)

    assert result.valid is False
    assert "Firewall radius hosts do not include RADIUS auth host" in result.errors


def test_detects_missing_portal_firewall_host():
    bundle = build_bundle()
    bad_firewall = bundle.firewall.model_copy(update={"portal_hosts": []})
    bad_bundle = bundle.model_copy(update={"firewall": bad_firewall})

    result = validate_provisioning_bundle(bad_bundle)

    assert result.valid is False
    assert any("Firewall portal hosts missing" in error for error in result.errors)


def test_warns_when_nat_disabled():
    result = validate_provisioning_bundle(build_bundle(nat_enabled=False))

    assert result.valid is True
    assert any("NAT is disabled" in warning for warning in result.warnings)
