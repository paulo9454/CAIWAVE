...
def test_renders_nat_masquerade_section():
    section = render_nat_section(build_bundle())

    assert section.name == RouterOSSectionName.NAT
    assert section.status == RenderStatus.RENDERED
    assert section.checksum
    assert "# CAIWAVE NAT" in section.content
    assert "# NAT strategy: masquerade" in section.content
    assert '/ip firewall nat add action="masquerade" chain="srcnat" comment="CAIWAVE managed hotspot masquerade" out-interface="ether1" src-address="10.10.0.0/24"' in section.content


def test_renders_nat_disabled_comment():
    section = render_nat_section(build_bundle(nat_enabled=False))

    assert "# NAT disabled by provisioning plan" in section.content
    assert "/ip firewall nat add" not in section.content
