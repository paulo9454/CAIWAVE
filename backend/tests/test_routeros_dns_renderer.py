...
def build_bundle():
    ...
    config = {
        ...
        "router_dns_upstreams": ["1.1.1.1", "8.8.8.8"],
...
def test_renders_dns_section():
    section = render_dns_section(build_bundle())

    assert section.name == RouterOSSectionName.DNS
    assert section.status == RenderStatus.RENDERED
    assert section.checksum
    assert "# CAIWAVE DNS" in section.content
    assert "# Captive DNS name: wifi.caiwave.com" in section.content
    assert '/ip dns set allow-remote-requests=yes servers="1.1.1.1,8.8.8.8"' in section.content
    assert '/ip dns static add address="10.10.0.1" comment="CAIWAVE managed captive portal DNS record" name="wifi.caiwave.com"' in section.content
