...
def build_bundle(lan_interfaces=None, create_bridge=True):
    router = {
        "id": "router-1",
        "name": "GOODlife",
        "owner_id": "owner-1",
        ...
        "lan_interfaces": lan_interfaces or ["ether2", "wlan1"],
        "create_bridge": create_bridge,
        "bridge_name": "bridge-hotspot",
        "effective_lan_interface": "bridge-hotspot" if create_bridge else "ether2",
        "mode": "fresh",
...
def test_renders_bridge_section():
    section = render_bridge_section(build_bundle())

    assert section.name == RouterOSSectionName.BRIDGE
    assert section.status == RenderStatus.RENDERED
    assert section.checksum
    assert "# CAIWAVE Bridge" in section.content
    assert '/interface bridge add comment="CAIWAVE managed hotspot bridge" name="bridge-hotspot"' in section.content


def test_renders_bridge_members():
    section = render_bridge_section(build_bundle())

    assert '/interface bridge port add bridge="bridge-hotspot" comment="CAIWAVE managed bridge member" interface="ether2"' in section.content
    assert '/interface bridge port add bridge="bridge-hotspot" comment="CAIWAVE managed bridge member" interface="wlan1"' in section.content


def test_renders_no_bridge_comment_for_none_action():
    bundle = build_bundle(lan_interfaces=["ether2"], create_bridge=False)

    assert bundle.bridge.action == BridgeAction.NONE

    section = render_bridge_section(bundle)

    assert "# No bridge required by topology plan" in section.content
    assert "/interface bridge add" not in section.content
