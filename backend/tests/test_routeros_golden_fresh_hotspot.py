...
GOLDEN_PATH = Path("backend/tests/golden/provisioning_v2/fresh_hotspot_router.rsc")
...
def test_fresh_hotspot_router_golden_file_is_stable():
    ...
    if not GOLDEN_PATH.exists():
        ...
        raise AssertionError(
            f"Golden file created at {GOLDEN_PATH}. Re-run the test to verify stability."
        )
...
def test_fresh_hotspot_router_artifact_contains_core_sections():
    artifact = render_routeros_bundle(bundle=build_bundle())
