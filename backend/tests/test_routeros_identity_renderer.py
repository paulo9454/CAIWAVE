...
def test_renders_identity_section():
    section = render_identity_section(build_bundle())

    assert section.name == RouterOSSectionName.IDENTITY
    assert section.status == RenderStatus.RENDERED
    assert section.checksum
    assert "# CAIWAVE Identity" in section.content
    assert '/system identity set name="GOODlife"' in section.content


def test_identity_section_includes_audit_comments():
    section = render_identity_section(build_bundle())

    assert "# CAIWAVE Provisioning Bundle: bundle:snapshot:router-1" in section.content
    assert "# Snapshot: snapshot:router-1" in section.content
    assert "# Router ID: router-1" in section.content
    assert "# Hotspot ID: hotspot-1" in section.content
