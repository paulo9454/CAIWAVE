...
from backend.services.provisioning_tracker import log_provisioning
from backend.services.mikrotik_builder import build_mikrotik_script
import datetime
import asyncio


def dispatch_router_provisioning(router: "RouterContext", mode: str = "script"):
    from backend.core.router_context import RouterContext
    if not isinstance(router, dict):
        router = router.model_dump()
    router = RouterContext(**router).model_dump()
