      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="dashboard-card">
          ...
          {chartData.length > 0 ? (
            ...
            <div className="h-64 flex items-center justify-center text-neutral-500">No area data available</div>
          )}
        </div>
        ...
        <div className="dashboard-card">
          ...
          {chartData.length > 0 ? (
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={chartData} cx="50%" cy="50%" outerRadius={80} dataKey="revenue" nameKey="fullName">
                    {chartData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.fill} />
                    ))}
                  </Pie>
                  <Tooltip contentStyle={{ backgroundColor: "#18181b", border: "1px solid #27272a", borderRadius: "8px" }} formatter={(value) => formatCurrency(value)} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          ) : (
            <div className="h-64 flex items-center justify-center text-neutral-500">No revenue data available</div>
          )}
        </div>
      </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {areaStats.areas.map((area, idx) => (
                  <tr key={area.constituency} className="border-b border-neutral-800/50 hover:bg-neutral-800/30">
                    ...
                    <td className="py-3 px-4"><span className="text-green-400">{area.active_hotspots}</span></td>
                    <td className="py-3 px-4 font-semibold">{area.total_sessions.toLocaleString()}</td>
                    <td className="py-3 px-4 font-semibold text-green-400">{formatCurrency(area.total_revenue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="dashboard-card">
          ...
          <div className="space-y-3">
            {rankings.rankings.slice(0, 5).map((hotspot) => (
              <div key={hotspot.id} className="flex items-center justify-between p-3 bg-neutral-800/50 rounded-lg">
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${
                    hotspot.rank === 1 ? "bg-yellow-500/20 text-yellow-400" :
                    hotspot.rank === 2 ? "bg-gray-500/20 text-gray-400" :
                    hotspot.rank === 3 ? "bg-orange-500/20 text-orange-400" : "bg-neutral-700 text-neutral-400"
                  }`}>
                    {hotspot.rank}
                  </div>
                  <div>
                    <p className="font-medium">{hotspot.name}</p>
                    <p className="text-sm text-neutral-500">{hotspot.location_name || hotspot.constituency}</p>
                  </div>
                </div>
