from backend.routes.mikrotik import mikrotik_router
"""
...
# =========================================================
# MikroTik Health Monitoring Configuration
# =========================================================

ROUTER_HEARTBEAT_TIMEOUT_MINUTES = int(
    os.environ.get("ROUTER_HEARTBEAT_TIMEOUT_MINUTES", "10")
)
...
router_monitor_task = None
    
@app.on_event("startup")
async def startup():
    # Prevent duplicate startup execution
    if getattr(app.state, "startup_initialized", False):
        return
...
async def initiate_payment(payment_data: PaymentCreate):
    ...
    if hotspot.get("status") not in ["active", None, HotspotStatus.PENDING_SETUP.value]:
        raise HTTPException(status_code=400, detail="Hotspot is not active")
...
class MikroTikRegisterRequest(BaseModel):
    ...
    bridge_name: Optional[str] = "bridge-hotspot"
...
def validate_lan(cls, v):
    ...
    def validate_dns(cls, v):
        v = (v or "").strip()
        if not v:
            raise ValueError("dns_name is required")
        return v
...
async def register_mikrotik(
    ...
    nas_id = generate_nas_id((request.name or "").strip())
    ...
    if not request.wan_interface:
        raise HTTPException(status_code=400, detail="wan_interface is required")

    if not request.wan_interface:
        raise HTTPException(status_code=400, detail="wan_interface is required")

    wan_interface = request.wan_interface.strip()

    lan_interfaces = [
    i.strip() for i in (request.lan_interfaces or [])
    if i and i.strip()
]

    if not lan_interfaces and not request.create_bridge:
        raise HTTPException(
            status_code=400,
            detail="At least one LAN interface is required when not using bridge mode"
        )

    # Validate WAN/LAN safety
    if wan_interface in lan_interfaces:
        raise HTTPException(status_code=400, detail="WAN interface cannot be in LAN interfaces")

    # Effective LAN interface
    if request.create_bridge:
            if not request.bridge_name:
                raise HTTPException(
                    status_code=400,
                    detail="bridge_name is required when create_bridge is true"
                )
            effective_lan_interface = request.bridge_name.strip()
    else:
        if len(lan_interfaces) != 1:
            raise HTTPException(
                status_code=400,
                detail="Existing mode requires exactly one LAN interface"
            )
        effective_lan_interface = lan_interfaces[0]

    # Derive network info
    ip_interface = ipaddress.ip_interface(request.hotspot_cidr)
    hotspot_network = str(ip_interface.network)
    hotspot_gateway = str(ip_interface.ip)

    # Validate DHCP pool
    pool_start, pool_end = request.dhcp_pool.split("-")
    pool_start_ip = ipaddress.ip_address(pool_start.strip())
    pool_end_ip = ipaddress.ip_address(pool_end.strip())

    if pool_start_ip not in ip_interface.network or pool_end_ip not in ip_interface.network:
        raise HTTPException(status_code=400, detail="DHCP pool must be inside hotspot network")

    if ip_interface.ip in [pool_start_ip, pool_end_ip]:
        raise HTTPException(status_code=400, detail="DHCP pool cannot include gateway IP")

    
    # Create router record
    # ================================
    # PROVISIONING SNAPSHOT (SOURCE OF TRUTH)
    # ================================
    ...
    router_record = {
        ...
        "provisioning_source": "owner_dashboard",
        ...
        # Derived topology
        "effective_lan_interface": effective_lan_interface,

        # Network contract
        "hotspot_cidr": request.hotspot_cidr,
       ...
       # Observability
        ...
        # Health monitoring (STEP 9 ADDITION)
...
async def confirm_mikrotik_connection(
    ...
    update_data = {
        "status": "connected",
        "heartbeat_received_at": datetime.now(timezone.utc).isoformat(),
...
from backend.services.mikrotik.router_engine import MikroTikRouterEngine
...
async def regenerate_mikrotik_script(
    ...
    try:
        result = MikroTikRouterEngine.generate(router)
    except Exception as e:
    ...
    single_rsc = generate_single_rsc_provisioning_file(
        router_name=normalized_router["name"],
        nas_id=normalized_router["nas_identifier"],
        radius_secret=normalized_router["radius_secret"],
        radius_host=RADIUS_HOST,
        callback_url=""
    )
...
@mikrotik_onboard_router.post("/health/evaluate")
async def evaluate_router_health():

    now = datetime.now(timezone.utc)

    routers = await db.mikrotik_routers.find({}, {"_id": 0}).to_list(None)
...
async def get_live_router_status(
    ...
    # Scope HOTSPOT_OWNER to their routers only
    if user["role"] == UserRole.HOTSPOT_OWNER.value:
        query["owner_id"] = user["id"]

    routers = await db.mikrotik_routers.find(query, {"_id": 0}).to_list(None)
    ...
    for router in routers:
        ...
        health = router.get("health_status", "unknown")
        ...
        if last_seen:
            try:
                ...
                is_online = delta <= 900  # 15 min threshold
                latency = int(delta)
            except Exception:
                is_online = False
        ...
        enriched.append({
            ...
            "firmware_version": router.get("firmware_version"),
...
# =========================================================
# ROUTER OFFLINE MONITOR
# =========================================================

# =========================================================
# ROUTER OFFLINE MONITOR (STEP 4)
# =========================================================

async def monitor_router_health():
    # Prevent double-start if called twice
    if getattr(app.state, "router_monitor_running", False):
        logger.warning("Router monitor already running, skipping duplicate start")
        return

    app.state.router_monitor_running = True

    logger.info("Router health monitor started")
    ...
    try:
        while True:
            try:
                ...
                result = await db.mikrotik_routers.update_many(
                    {
                        "heartbeat_received_at": {"$lt": cutoff},
...
async def get_dashboard_stats(user: dict = Depends(get_current_user)):
    if user["role"] == UserRole.HOTSPOT_OWNER.value:
        ...
        return {
            ...
            "active_hotspots": len([h for h in hotspots if h.get("status") == "active"]),
        ...
        active_hotspots = await db.hotspots.count_documents({"status": "active"})
...
async def admin_suspend_overdue(user: dict = Depends(require_admin)):
    ...
    owner_ids = list(set(
    inv["owner_id"]["id"] if isinstance(inv["owner_id"], dict)
    else inv["owner_id"]
