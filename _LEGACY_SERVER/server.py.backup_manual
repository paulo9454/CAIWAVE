from backend.services.hotspot.hotspot_service import HotspotService
from dotenv import load_dotenv
from backend.routes.provisioning_status import status_router
...
async def get_hotspots():
    hotspots = await HotspotService.list_hotspots(owner_id=None)
    return {"hotspots": hotspots}

@app.get("/packages")
async def get_packages():
    packages = await PackageService.list_active()
...
@app.get("/ads")
async def get_ads():
    ads = await AdService.list_active()
    return {"ads": ads}
...
async def create_free_session(hotspot_id: str, ad_id: str, user_mac: str = None, user_ip: str = None):
    ...
    free_count = await SessionService.count_documents({
        "is_free": True,
    ...
    ad = await AdService.get_active_one({"id": ad_id, "is_active": True})
    if not ad:
    ...
    await SessionService.insert(session)
...
async def free_session_status(hotspot_id: str, user_mac: str = None, user_ip: str = None):
    ...
    sessions = await SessionService.find({
        "hotspot_id": hotspot_id,
        "$or": [
            {"user_mac": user_identifier},
            {"user_ip": user_identifier}
...
async def get_payments():
    payments = await PaymentService.list_all()
    return {"payments": payments}

@app.get("/api/payments/{phone}")
async def get_payment_by_phone(phone: str):
    payments = await PaymentService.find_by_phone(phone)
    return {"payments": payments}
...
async def activate_hotspot_access(phone: str, amount: float):
    ...
    package = await PackageService.get_one(
...
# AUTH ROUTER (FIXED IMPORT ORDER)
